...
pacman::p_load(R2jags, parallel, ggplot2, stats)


set.seed(1983)

### NB! Don't forget to set your working directory
setwd("/work/desccionmaking/mousedata/copy_module_3(1)/predictive_checks")
...
#load control data
ctr_data <- read.table("/work/desccionmaking/mousedata/copy_module_3(1)/data/IGT_sober_mouse.txt",header=TRUE)
#899 choices per mouse
...
temp_samples <- jags.parallel(data, inits=NULL, params,
                              model.file ="/work/desccionmaking/mousedata/copy_module_3(1)/PVL/PVL.txt",
...
plot(density(temp_samples$BUGSoutput$sims.list$w), main="Density w")
plot(density(temp_samples$BUGSoutput$sims.list$A), main="Density A")
plot(density(temp_samples$BUGSoutput$sims.list$theta), main="Density theta")
plot(density(temp_samples$BUGSoutput$sims.list$a), main="Density a")
...
library(modeest)  # For mode estimation

# Extract samples for a parameter (e.g., "param_name")
samples <- temp_samples$BUGSoutput$sims.list$theta 

# Compute MAP (mode of the posterior)
map_value <- density(samples)$x[which.max(density(samples)$y)]  # Mode estimate from density
print(map_value)


# Compute 95% credible interval (2.5% and 97.5% quantiles)
credible_interval <- quantile(samples, probs = c(0.025, 0.975))

# Print results
cat("MAP Estimate:", map_value, "\n")
...
# Convert JAGS output to an mcmc.list object
mcmc_samples <- as.mcmc(temp_samples)

# Plot trace plots for all parameters
par(mfrow=c(2,2))  # 2x2 layout
traceplot(mcmc_samples[, "w"], main="Trace Plot: w")
traceplot(mcmc_samples[, "A"], main="Trace Plot: A")
traceplot(mcmc_samples[, "theta"], main="Trace Plot: theta")
traceplot(mcmc_samples[, "a"], main="Trace Plot: a")


# gets the r hat value to look at it in numbers
